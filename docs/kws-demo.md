...
先准备一个 16-bit、单声道 WAV 文件，内容包含“小余小余”。然后运行：

```bash
node demos/kws-file-demo.js \
  --wav ./your-xiaoyu.wav \
  --keywords models/kws/keywords-xiaoyu.txt
```

当前自定义关键词文件：

```text
models/kws/keywords-xiaoyu.txt
```

内容：

```text
x iǎo y ú x iǎo y ú :2.0 #0.45 @小余小余
```
...
sherpa-onnx-cli text2token \
  ...
  --tokens-type ppinyin \
  models/kws/keywords-raw-xiaoyu.txt \
  models/kws/keywords-xiaoyu.txt
```
